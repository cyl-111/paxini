...
class HighSpeedCommBoard:
    def __init__(self, root):
        ...
        self.root.geometry("1300x800")  # 稍微调小了高度，因为精简了界面
        ...
        self.distribution_addr_ranges = {
            (0x1000, 0x11FF): "大拇指近节", (0x1200, 0x13FF): "大拇指中节",
            (0x1400, 0x15FF): "大拇指指尖", (0x1600, 0x17FF): "大拇指指甲",
            (0x1800, 0x19FF): "食指近节", (0x1A00, 0x1BFF): "食指中节",
            (0x1C00, 0x1DFF): "食指指尖", (0x1E00, 0x1FFF): "食指指甲",
            (0x2000, 0x21FF): "中指近节", (0x2200, 0x23FF): "中指中节",
            (0x2400, 0x25FF): "中指指尖", (0x2600, 0x27FF): "中指指甲",
            (0x2800, 0x29FF): "无名指近节", (0x2A00, 0x2BFF): "无名指中节",
            (0x2C00, 0x2DFF): "无名指指尖", (0x2E00, 0x2FFF): "无名指指甲",
            (0x3000, 0x31FF): "小拇指近节", (0x3200, 0x33FF): "小拇指中节",
            (0x3400, 0x35FF): "小拇指指尖", (0x3600, 0x37FF): "小拇指指甲",
            (0x3800, 0x38FF): "掌心1", (0x3900, 0x39FF): "掌心2",
            (0x3A00, 0x3AFF): "掌心3", (0x3B00, 0x3BFF): "掌心4",
            (0x3C00, 0x3CFF): "掌心5", (0x3D00, 0x3DFF): "掌心6",
            (0x3E00, 0x3EFF): "掌心7", (0x3F00, 0x3FFF): "掌心8"
        }
    ...
    def init_ui(self):
        ...
        ttk.Button(config_frame, text="获取版本号", command=self.get_version).grid(row=0, column=9, padx=5, pady=5)
        ttk.Button(config_frame, text="清空日志", command=self.clear_log).grid(row=0, column=10, padx=5, pady=5)
    ...
    def read_connected_sensors(self):
        ...
        for sensor in self.connected_sensors:
            if sensor.startswith("大拇指"): groups["大拇指"].append(sensor)
            elif sensor.startswith("食指"): groups["食指"].append(sensor)
            elif sensor.startswith("中指"): groups["中指"].append(sensor)
            elif sensor.startswith("无名指"): groups["无名指"].append(sensor)
            elif sensor.startswith("小拇指"): groups["小拇指"].append(sensor)
            elif sensor.startswith("掌心"): groups["掌心"].append(sensor)
                    ...
                    try:
                        ...
                        if parsed and parsed["func_code"] == 0x03 and parsed["data_len"] == 2:
                            ...
                            if data_len > max_possible_len:
                                self.log(f"  {sensor}: 数据长度({data_len})超过地址范围最大长度，将截断读取")
    ...
    def parse_module_forces(self, data: bytes) -> List[Dict]:
        ...
        for i in range(valid_modules):
            ...
            parsed.append({
                ...
                "raw_hex": (fx_bytes.hex().upper(), fy_bytes.hex().upper(), fz_bytes.hex().upper()),
                "used_byte": (f"0x{fx_low_byte:02X}", f"0x{fy_low_byte:02X}", f"0x{fz_low_byte:02X}"),
        ...
        if parsed:
            self.log("\n===== 大拇指传感器 =====")
            for module in parsed[0:4]: self.log_module_force(module)
            self.log("\n===== 食指传感器 =====")
            for module in parsed[4:8]: self.log_module_force(module)
            self.log("\n===== 中指传感器 =====")
            for module in parsed[8:12]: self.log_module_force(module)
            self.log("\n===== 无名指传感器 =====")
            for module in parsed[12:16]: self.log_module_force(module)
            self.log("\n===== 小拇指传感器 =====")
            for module in parsed[16:20]: self.log_module_force(module)
            self.log("\n===== 掌心传感器 =====")
            for module in parsed[20:28]: self.log_module_force(module)
        else:
    ...
    def read_module_forces(self, cycle_mode=False):
        ...
        try:
            ...
            if not response:
                ...
                if cycle_mode: self.root.after(50, self.read_module_forces, True)
                return
    ...
    def read_registers(self, cycle_mode=False):
        ...
        try:
            ...
            if read_len > 512:
                ...
                if cycle_mode: self.root.after(50, self.read_registers, True)
                return
            request_frame = self.build_request_frame(0x03, reg_addr, b"")
            self.ser.write(request_frame)
